...
const AWAY_MS   = 30 * 60 * 1000  // 30 minutes → away
    ...
    <div className="space-y-8">
      ...
      {!hasMembersOrInvites ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-700 bg-zinc-900 py-20">
          ...
          <p className="text-sm font-medium text-zinc-400">No members yet</p>
        ...
        <>
          {/* Members grouped by app */}
          {apps.filter((a) => a.members.length > 0).map((app) => {
            const appOnline = app.members.filter(
              (m) => presenceState(m.user.lastSeenAt) === 'online'
            ).length
                ...
                <div className="flex items-center justify-between border-b border-zinc-800 px-5 py-3.5">
                  <div className="flex items-center gap-3">
                    ...
                    <Link
                      ...
                      className="text-sm font-semibold text-zinc-100 hover:text-indigo-400"
                    >
                      {app.name}
                    </Link>
                    <span className="text-xs text-zinc-500">
                      {app.members.length} member{app.members.length !== 1 ? 's' : ''}
                    </span>
                  </div>
                  {appOnline > 0 && (
                      ...
                      <div key={m.userId} className="flex items-center gap-4 px-5 py-3.5">
                        ...
                        <div className="hidden shrink-0 text-right sm:block">
                          {state === 'online' ? (
                            ...
                            <span className="text-xs text-zinc-600" title={formatDateTime(m.user.lastSeenAt)}>
                              {relativeTime(m.user.lastSeenAt)}
                            </span>
                          ) : (
                            <span className="text-xs text-zinc-700">Never seen</span>
                          )}
                        </div>
                      </div>
                    )
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-800 bg-zinc-800/40">
                    <th className="px-5 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-zinc-500">Email</th>
                    <th className="px-5 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-zinc-500">Application</th>
                    <th className="px-5 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-zinc-500">Role</th>
                    <th className="px-5 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-zinc-500">Sent</th>
